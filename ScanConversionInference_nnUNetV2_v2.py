...
def ScanConversionInference():
    ...
    parser.add_argument("--mask-class", type=str, default="kidney", choices=["kidney", "calyx"],help="Choose which segmentation mask to send back"),
...
def run_client(args):
    ...
    while True:
        ...
        if image_ready and tfm_ready:
            ...
            prediction = postprocess_prediction(prediction, original_size=orig_img_size, mask_selection=args.mask_class, scanconversion_config=scanconversion_config,vertices=vertices,weights=weights, mask_array=mask_array)
...
# Postprocess prediction modified to select a single class mask
def postprocess_prediction(
    ...
    Returns:
        mask_out: uint8 array [1, H, W], representing the requested class.
    """
    ...
    if logits.ndim != 3 or logits.shape[0] < 3:
        raise ValueError(f"Unexpected prediction shape {logits.shape}, expected [3, H, W]")

    # ------------------------------
    # 2. Define class indices
    # ------------------------------
    CLASS_KIDNEY = 1
    CLASS_CALYX  = 2

    if mask_selection == "kidney":
        target_class = CLASS_KIDNEY
    elif mask_selection == "calyx":
        target_class = CLASS_CALYX
    else:
        raise ValueError(f"Invalid mask_selection '{mask_selection}'. Expected 'kidney' or 'calyx'.")

    # ------------------------------
    # 3. Compute class mask via argmax
    # ------------------------------
    label_map = np.argmax(logits, axis=0)    # [H, W]

    binary_mask = (label_map == target_class).astype(np.uint8) * 255  # ensure correct scaling

    # ------------------------------
    # 4. Optional scan conversion
    # ------------------------------
    if scanconversion_config is not None:
