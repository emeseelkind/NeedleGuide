"""
From https://github.com/SlicerIGT/aigt/tree/master
SlicerExtension/LiveUltrasoundAi/TorchSequenceSegmentation
Edited for use in KidneyNav
"""
...
# nnUNet V2 for .pth checkpoint inference (optional)
try:
...
def _load_nnunet_config(plans_path):
    """
    Load nnUNet plans file (config.json or plans.json) and extract configuration parameters.
    Mirrors NeedleGuide ScanConversionInference_nnUNetV2_v2.load_nnunet_config.
    """
    ...
    patch_size = config_2d['patch_size']
...
class TorchSequenceSegmentationLogic(ScriptedLoadableModuleLogic):
    ...
    def __init__(self):
        ...
        self.model = None
        self.metadata = None
        self._nnunet_config = None  # set when loading nnUNet .pth checkpoint (Z-score params)
    ...
    def setDefaultParameters(self, parameterNode):
        ...
        if not parameterNode.GetParameter("UseNnUNetCheckpoint"):
            parameterNode.SetParameter("UseNnUNetCheckpoint", "false")
    ...
    def getAllModelPaths(self):
        ...
        if modelFolder:
            if os.path.exists(os.path.join(modelFolder, "configs/metadata.json")):
                ...
                models = glob.glob(os.path.join(modelFolder, "**", "*.pth"), recursive=True)
    ...
    def loadModel(self, modelPath):
        ...
        if not modelPath:
            ...
            if parameterNode.GetParameter("UseMonaiBundle") == "true":
                ...
                plans_path = os.path.join(model_dir, "config.json") if os.path.isfile(os.path.join(model_dir, "config.json")) else (os.path.join(model_dir, "plans.json") if os.path.isfile(os.path.join(model_dir, "plans.json")) else None)
                is_pth = modelPath.lower().endswith(".pth")

                if plans_path and is_pth:
                    # nnUNet checkpoint: build network from plans and load state_dict
                    if not NNUNET_AVAILABLE:
                        logging.warning("nnUNet V2 is required for .pth checkpoints with config.json/plans.json. Install with: pip install nnunetv2")
                        self.model = None
                        self._nnunet_config = None
                        parameterNode.SetParameter("UseNnUNetCheckpoint", "false")
                    else:
                        try:
                            config = _load_nnunet_config(plans_path)
                            output_channels = 2  # binary segmentation (background + foreground); override if plans specify
                            model = get_network_from_plans(
                                config['arch_class_name'],
                                config['arch_kwargs'],
                                config['arch_kwargs_req_import'],
                                input_channels=1,
                            ...
                            model.eval()
                            self.model = model
                            self._nnunet_config = config
                            parameterNode.SetParameter("UseNnUNetCheckpoint", "true")
                            patch_size = config['patch_size']
                            parameterNode.SetParameter("ModelInputSize", str(patch_size[0]))
                            parameterNode.SetParameter("WindowSize", str(patch_size[0]))
                            parameterNode.SetParameter("TrackingMethod", "None")
                            logging.info("Loaded nnUNet checkpoint (2D); patch_size=%s", patch_size)
                        except Exception as e:
                            logging.error("Failed to load nnUNet checkpoint: %s", e)
                            logging.info(traceback.format_exc())
                            self.model = None
                            self._nnunet_config = None
                            parameterNode.SetParameter("UseNnUNetCheckpoint", "false")
                else:
                    # TorchScript / legacy .pt (or .pth without plans)
                    parameterNode.SetParameter("UseNnUNetCheckpoint", "false")
                    self._nnunet_config = None
                    ...
                    try:
                        ...
                        logging.warning("TorchScript load failed; trying torch.load for legacy checkpoint")
                        self.model = torch.load(modelPath, map_location=DEVICE, weights_only=False)
                        if hasattr(self.model, 'eval'):
                            self.model.eval()
                    else:
                        self.model.eval()

                    raw_config = extra_files.get("config.json") or b"" or ""
                    config_json = raw_config.decode("utf-8") if isinstance(raw_config, bytes) else (raw_config or "")
                    if config_json:
                        try:
                            config = json.loads(config_json) if isinstance(config_json, str) else config_json
                            config_2d = config.get("configurations", {}).get("2d", {})
                            if config_2d:
                                patch_size = config_2d.get('patch_size')
                                if patch_size:
                                    parameterNode.SetParameter("ModelInputSize", str(patch_size[0]))
                                    parameterNode.SetParameter("WindowSize", str(patch_size[0]))
                            try:
                                useTrackingLayer = config.get("use_tracking_layer")
                                if useTrackingLayer:
                                    if config.get("tracking_method") == "local":
                                        parameterNode.SetParameter("TrackingMethod", "Local")
                                        parameterNode.SetParameter("WindowTargetFrame", str(config.get("window_target_frame", 0)))
                                        parameterNode.SetParameter("ImagePixelNorm", str(config.get("orig_img_size", 256)))
                                    elif config.get("tracking_method") == "global":
                                        if config.get("use_identity"):
    ...
    def getPrediction(self, inputArray, inputTfmArray=None):
        ...
        if parameterNode.GetParameter("UseMonaiBundle") == "true":
            ...
            if parameterNode.GetParameter("UseNnUNetCheckpoint") == "true" and self._nnunet_config:
                # nnUNet: Z-score normalization from plans (mirror NeedleGuide)
                norm_mean = self._nnunet_config['normalization_mean']
                norm_std = self._nnunet_config['normalization_std']
                imageTensor = (imageTensor - norm_mean) / (norm_std if norm_std else 1.0)
            else:
                # TorchScript / legacy: optional 0-1 normalization
                normalizeInput = slicer.util.settingsValue(self.LAST_NORMALIZE_SETTING, False, converter=slicer.util.toBool)
                if normalizeInput:
                    if imageTensor.max() <= 1.0:
        ...
        with torch.inference_mode():
            if parameterNode.GetParameter("UseMonaiBundle") == "true":
                ...
                if inputTfmArray is not None:
                    output = self.model((imageTensor, inputTfmTensor))
                else:
                    output = self.model(imageTensor)
        ...
        if recordAsSegmentation:
            ...
            if not segSeqBr:
                ...
                while not refTransformFound:
                    ...
                    if not parentNode:
                        logging.warning(
                            "TorchSequenceSegmentation: reached transform root without finding "
                            "ImageToReference ('ToRef' in name); segmentation browser may be incomplete."
                        )
                        break
            ...
            if segmentationNode.GetSegmentation().GetNumberOfSegments() == 0:
                ...
                if not segmentId or segmentationNode.GetSegmentation().GetSegment(segmentId) is None:
                    segmentIds = vtk.vtkStringArray()
                    segmentationNode.GetSegmentation().GetSegmentIDs(segmentIds)
                    ...
                    parameterNode.SetParameter("SegmentId", segmentId)
            ids.InsertNextValue(segmentId)

            if segSeqBr.GetSequenceNode(segmentationNode) is None:
        ...
        for itemIndex in range(sequenceBrowser.GetNumberOfItems()):
            ...
            if recordAsSegmentation:
                ...
                if itemIndex % (numSkipFrames + 1) == 0:
                    ...
                    segSeqBr.SetSelectedItemNumber(itemIndex)
