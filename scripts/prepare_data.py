...
# Apply defaults for new options if they are missing
config.setdefault("label_encoding", "single_labelmap")
config.setdefault("overlap_policy", "priority")
config.setdefault("class_order", None)
...
def normalize_class_order(class_order, num_channels):
    if class_order is None:
        return list(range(num_channels))
    normalized = [int(value) for value in class_order]
    if len(normalized) != num_channels:
        logging.error("class_order length must match number of channels")
        sys.exit(1)
    if min(normalized) == 1 and max(normalized) == num_channels:
        normalized = [value - 1 for value in normalized]
    if sorted(normalized) != list(range(num_channels)):
        logging.error("class_order must contain all channel indices exactly once")
        sys.exit(1)
    return normalized


def convert_multichannel_to_labelmap(segmentation_data, class_order, overlap_policy):
    num_frames, rows, cols, num_channels = segmentation_data.shape
    dtype = np.uint16 if num_channels > 255 else np.uint8
    normalized_order = normalize_class_order(class_order, num_channels)

    overlap_count = np.sum(np.sum(segmentation_data > 0, axis=-1) > 1)
    if overlap_count > 0:
        logging.info(f"Found {overlap_count} overlapping pixels across channels")

    if overlap_policy == "priority":
        labelmap = np.zeros((num_frames, rows, cols), dtype=dtype)
        for channel_index in normalized_order:
            mask = segmentation_data[:, :, :, channel_index] > 0
            labelmap[(labelmap == 0) & mask] = channel_index + 1
    elif overlap_policy == "argmax":
        any_mask = np.any(segmentation_data > 0, axis=-1)
        argmax = np.argmax(segmentation_data, axis=-1)
        labelmap = np.where(any_mask, argmax + 1, 0).astype(dtype)
    else:
        logging.error(f"Unknown overlap_policy: {overlap_policy}")
        sys.exit(1)
    ...
    labels_info = {
        ...
        "class_ids_by_channel_index": {str(i + 1): int(i + 1) for i in range(num_channels)},
        "class_order_channel_indices": [int(index + 1) for index in normalized_order],
        "overlap_policy": overlap_policy,
    }
    return labelmap, labels_info
...
for seg_filename in tqdm(data_files):
    ...
    if segmentation_data.ndim == 3:
        segmentation_data = segmentation_data[:, :, :, np.newaxis]
    if segmentation_data.ndim != 4:
        logging.error(f"Segmentation data must be 4D (N,H,W,C). Got shape {segmentation_data.shape}")
        sys.exit(1)
    ...
    if os.path.exists(indices_filename):
        ...
        indices = np.arange(segmentation_data.shape[0], dtype=np.int32)
    ...
    labels_info = {
        ...
        "num_channels": int(segmentation_data.shape[3]),
        "class_ids_by_channel_index": {str(i + 1): int(i + 1) for i in range(segmentation_data.shape[3])},
    }

    if label_encoding == "single_labelmap":
        labelmap_data, labelmap_info = convert_multichannel_to_labelmap(
            segmentation_data, class_order, overlap_policy
        )
        labels_info.update(labelmap_info)
        segmentation_to_resize = labelmap_data
        output_channels = 1
        output_dtype = labelmap_data.dtype
    elif label_encoding == "multi_channel":
        segmentation_to_resize = segmentation_data
        output_channels = segmentation_data.shape[3]
        output_dtype = segmentation_data.dtype
    else:
        logging.error(f"Unknown label_encoding: {label_encoding}")
        sys.exit(1)

    # Resize segmentation images channel by channel.

    resized_data = np.zeros(
        (segmentation_data.shape[0], config["image_size"], config["image_size"], output_channels),
        dtype=output_dtype,
    )
    if label_encoding == "single_labelmap":
        for i in range(segmentation_data.shape[0]):
            resized_data[i, :, :, 0] = cv2.resize(
                segmentation_to_resize[i, :, :],
        ...
        for i in range(segmentation_data.shape[0]):
            for j in range(segmentation_data.shape[3]):
                resized_data[i, :, :, j] = cv2.resize(
                    segmentation_to_resize[i, :, :, j],
                    (config["image_size"], config["image_size"]),
                    interpolation=cv2.INTER_NEAREST,
                )
    ...
    labels_info_filename = os.path.splitext(os.path.basename(seg_filename))[0] + "_labels_info.yaml"
    labels_info_path = os.path.join(args.output_dir, labels_info_filename)
    with open(labels_info_path, "w") as f:
        yaml.dump(labels_info, f)
    logging.info(f"Saved labels metadata to {labels_info_path}")
